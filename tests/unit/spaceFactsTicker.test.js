...
import SpaceFactsTicker, { SPACE_FACTS } from '../../src/ui/SpaceFactsTicker.js';
...
describe('SpaceFactsTicker Component', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should create content container', () => {
      ...
      expect(ticker.tickerContent.className).toBe('ticker-content');
    });
  ...
  describe('Custom Facts', () => {
    ...
    test('should display custom facts in rotation', () => {
      ...
      // Cycle through all original facts
      const factCount = ticker.facts.length;
      for (let i = 0; i < factCount; i++) {
        jest.advanceTimersByTime(ticker.rotationDelay);
      }
      ...
      expect(ticker.tickerContent.textContent).toContain('Test fact from custom');
    });
  });
