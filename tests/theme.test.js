...
describe('Theme & Glassmorphism Styles - CSS File Verification', () => {
  ...
  describe('Glassmorphism Styles', () => {
    ...
    test('should have .glass-panel class with padding', () => {
      expect(cssContent).toMatch(/\.glass-panel\s*\{[^}]*padding\s*:\s*var\(--space-lg\)/);
    });
    ...
    test('should have .glass-intense variant with stronger blur', () => {
      expect(cssContent).toMatch(/\.glass-intense\s*\{[^}]*backdrop-filter\s*:\s*blur\(20px\)/);
    });

    test('should have .glass-light variant', () => {
      expect(cssContent).toMatch(/\.glass-light\s*\{[^}]*backdrop-filter\s*:\s*blur\(8px\)/);
    });
  });
  ...
  describe('Typography Styles', () => {
    test('should have h1-h6 selectors with font-game', () => {
      expect(cssContent).toMatch(/h1,\s*h2,\s*h3,\s*h4,\s*h5,\s*h6\s*\{[^}]*font-family\s*:\s*var\(--font-game\)/);
    });
    ...
    test('should have paragraph styles with data font', () => {
      expect(cssContent).toMatch(/p\s*\{[^}]*font-family\s*:\s*var\(--font-data\)/);
    });
    ...
    test('should apply text-shadow to headings for glow effect', () => {
      expect(cssContent).toMatch(/h[1-6]\s*\{[^}]*text-shadow\s*:\s*0\s*0\s*10px/);
    });
  });
  ...
  describe('Base Layout & Container Styles', () => {
    ...
    test('should have .container class with flex', () => {
      ...
      expect(cssContent).toMatch(/\.container\s*\{[^}]*flex-direction\s*:\s*column/);
    });

    test('should have .container-full for fullscreen', () => {
      expect(cssContent).toMatch(/\.container-full\s*\{[^}]*width\s*:\s*100vw/);
      expect(cssContent).toMatch(/\.container-full\s*\{[^}]*height\s*:\s*100vh/);
    });

    test('should have .grid classes', () => {
      expect(cssContent).toMatch(/\.grid\s*\{[^}]*display\s*:\s*grid/);
      expect(cssContent).toMatch(/\.grid-2\s*\{[^}]*grid-template-columns\s*:\s*repeat\(2/);
      expect(cssContent).toMatch(/\.grid-3\s*\{[^}]*grid-template-columns\s*:\s*repeat\(3/);
      expect(cssContent).toMatch(/\.grid-4\s*\{[^}]*grid-template-columns\s*:\s*repeat\(4/);
    });

    test('should have .flex classes', () => {
      expect(cssContent).toMatch(/\.flex\s*\{[^}]*display\s*:\s*flex/);
      expect(cssContent).toMatch(/\.flex-col\s*\{[^}]*flex-direction\s*:\s*column/);
      expect(cssContent).toMatch(/\.flex-center\s*\{/);
      expect(cssContent).toMatch(/\.flex-between\s*\{/);
    });

    test('should have .header styles', () => {
      expect(cssContent).toMatch(/\.header\s*\{[^}]*padding\s*:\s*var\(--space-lg\)/);
      expect(cssContent).toMatch(/\.header\s*\{[^}]*border-bottom/);
    });

    test('should have .footer styles', () => {
      expect(cssContent).toMatch(/\.footer\s*\{[^}]*padding\s*:\s*var\(--space-md\)/);
    ...
    test('should have .section class with spacing', () => {
      expect(cssContent).toMatch(/\.section\s*\{[^}]*padding\s*:\s*var\(--space-xl\)/);
  ...
  describe('Dark Space Theme Implementation', () => {
    test('body should have dark background', () => {
      expect(cssContent).toMatch(/body\s*\{[^}]*background-color\s*:\s*var\(--color-bg-primary\)/);
    });

    test('should have text color utilities', () => {
      expect(cssContent).toMatch(/\.text-primary\s*\{[^}]*color\s*:\s*var\(--color-text-primary\)/);
    ...
    test('should have accent color utilities', () => {
      expect(cssContent).toMatch(/\.text-accent-cyan\s*\{[^}]*color\s*:\s*var\(--color-accent-cyan\)/);
    ...
    test('should have background color utilities', () => {
      expect(cssContent).toMatch(/\.bg-primary\s*\{[^}]*background-color\s*:\s*var\(--color-bg-primary\)/);
      expect(cssContent).toMatch(/\.bg-secondary\s*\{/);
      expect(cssContent).toMatch(/\.bg-tertiary\s*\{/);
    });

    test('should have shadow utilities', () => {
      expect(cssContent).toMatch(/\.shadow-sm\s*\{[^}]*box-shadow\s*:\s*var\(--shadow-sm\)/);
      expect(cssContent).toMatch(/\.shadow-md\s*\{/);
      expect(cssContent).toMatch(/\.shadow-lg\s*\{/);
      expect(cssContent).toMatch(/\.shadow-glass\s*\{/);
    });
    ...
    test('should have spacing utilities', () => {
      expect(cssContent).toMatch(/\.m-md\s*\{[^}]*margin\s*:\s*var\(--space-md\)/);
      expect(cssContent).toMatch(/\.p-md\s*\{[^}]*padding\s*:\s*var\(--space-md\)/);
      expect(cssContent).toMatch(/\.mt-md\s*\{[^}]*margin-top\s*:\s*var\(--space-md\)/);
      expect(cssContent).toMatch(/\.mb-md\s*\{[^}]*margin-bottom\s*:\s*var\(--space-md\)/);
    });
  });
...
describe('HTML Integration - CSS Link', () => {
  ...
  test('index.html should link to styles/main.css', () => {
    expect(htmlContent).toMatch(/<link[^>]*href\s*=\s*["\']\.\/styles\/main\.css["\']/);
  ...
  test('should not have inline styles for canvas (deferred to CSS)', () => {
    ...
    const inlineStyleCount = (htmlContent.match(/<(body|html)[^>]*style\s*=/g) || []).length;
